...
struct CliParser::impl_t
{
    ...
    void setup()
    {
        ...
        generateCmd->add_option("--import-paths", generate.importPaths, "Additional .proto directories for imports lookup")
            ->check(CLI::ExistingDirectory);
        generateCmd->add_option("--output,-o", generate.outputDir, "LuaCATS file destination, defaults to current directory")
            ->check(CLI::ExistingDirectory);
        generateCmd->callback([this]() { selected = Command::Generate; });
        ...
        runCmd->callback([this]() { selected = Command::Run; });
    }
};
