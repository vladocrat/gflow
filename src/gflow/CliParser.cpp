...
struct CliParser::impl_t
{
    ...
    void setup()
    {
        ...
        runCmd->add_option("--address,-a", run.address, "Host address in an IPv4 format")->capture_default_str();
        ...
        runCmd->add_option("--proto", run.protoFile, "Path to a .proto file")->required()->check(CLI::ExistingFile);
