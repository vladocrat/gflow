...
# then produces dist/gflow-<version>-linux-<arch>.tar.gz.
#
# On Linux the default vcpkg triplet is static, so the gflow binary needs no
...
pkg_name="gflow-${version}-linux-${arch}"
